...
template <typename T> bool Stack<T>::stackFull() {
  if (m_iTop == m_iSize) {
    return true;
  } else {
    return false;
...
template <typename T> void Stack<T>::clearStack() { m_iTop = 0; }

template <typename T> int Stack<T>::stackLength() { return m_iTop; }

template <typename T> bool Stack<T>::push(T elem) {
  if (stackFull()) {
    return false;
  }
...
template <typename T> void Stack<T>::stackTraverse(bool isFromBottom) {
  if (isFromBottom) {
